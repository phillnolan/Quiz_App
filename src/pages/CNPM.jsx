...
const quizModules = import.meta.glob('../data/CNPM/*.js');
const quizzes = Object.keys(quizModules).map((path) => {
    const match = path.match(/Quiz(\d+)\.js$/);
    return match ? { id: match[1], name: `Đề ${match[1]}`, href: `/CNPM/${match[1]}` } : null;
}).filter(Boolean);


export default function CNPM() {
    return (
            ...
            <div className="max-w-4xl mx-auto">
                <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center">
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8'>
                        <Link to="/CNPM/random" className="block">
